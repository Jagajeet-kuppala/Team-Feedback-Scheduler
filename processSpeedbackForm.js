...
function tableCreator(userPairs) {
    ...
    for(let pair = 0; pair < userPairs.length; ++pair) {
        const user1 = (userPairs[pair][0] === PropertiesService.getScriptProperties().getProperty('DUMMY_USER')) ? "TIMEKEEPER" : userPairs[pair][0];
        const user2 = (userPairs[pair][1] === PropertiesService.getScriptProperties().getProperty('DUMMY_USER')) ? "TIMEKEEPER" : userPairs[pair][1];
